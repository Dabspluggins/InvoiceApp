-- Tighten the client_id check in apply_client_credit so that invoices with
-- a NULL client_id are also rejected. Previously only a non-null mismatch
-- raised an exception, letting old invoices bypass the ownership check.
...
begin
  ...
  -- Require an exact client_id match; NULL is also rejected so old invoices
  -- without a linked client cannot have credit applied.
  if v_invoice.client_id is null or v_invoice.client_id <> p_client_id then
